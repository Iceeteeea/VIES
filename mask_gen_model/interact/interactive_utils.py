...
from mask_gen_model.dataset.range_transform import im_normalization
...
def load_images(path, min_side=None):
    # load_images：将输入的图片转化固定尺寸的 np.array --> 堆叠起来：(t, ... )
    ...
    for i, fname in enumerate(fnames):
        if min_side:
            ...
            w, h = image.size
            # 这里是将 w,h 调整到统一的尺寸，比如 w>h, 若 h=480, 则 w = 480*(w//h)
