...
from main_win.win_v2.main_win_v7 import Ui_MainWindow
from main_win.win_v2.sub1_win_v7_1 import Ui_Dialog_tool
from main_win.win_v2.sub2_win_v7 import Ui_Dialog_Mask
# 视频修复的相关模型
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    pervious_frame_signal = pyqtSignal()
    next_frame_signal = pyqtSignal()
    to_SN_Slider_signal = pyqtSignal()
    ...
    def Btn_Start_Stop(self):
        if self.is_pause or self.is_switching:
            self.start_stop_pushButton.setText('暂停')
            self.is_pause = False
            self.Btn_Start()
        elif (not self.is_pause) and (not self.is_switching):
            self.start_stop_pushButton.setText('开始')
            self.is_pause = True
            self.Btn_Stop()
    ...
    # 只要self.timer_camera定时器启动，就一直播放。
    # 功能完成前，只播放原图；完成后，播放原图和相应完成的图片
    def Open_Frame(self):
        ...
        # todo 23.4.15 窗口2 bug:在窗口1播放的同时，点击修复，会出现bug，因为修复没完成，窗口2播放，但找不到文件
        # todo 建议窗口 1与窗口2 的播放需要分开
        # (2) 修复的视频帧播放
        # 判断是否有相关功能完成了，若完成了则播放相应的结果；
        # 不需要判别是哪一种功能完成了，因为结果都存在了completed_frames_root里面
        # print('self.inp_deno_deg_sr_inter_deold_stab_flags:', self.inp_deno_deg_sr_inter_deold_stab_flags)
        if sum(self.inp_deno_deg_sr_inter_deold_stab_flags) > 0:
            completed_frames_dir = os.path.join(self.completed_frames_path, self.completed_frames_name_list[self.index])
            ...
            if True:
                if len(completed_image_show.shape) == 3:
                    ...
                    video_completed_img = QImage(completed_image_show.data, completed_image_show.shape[1], completed_image_show.shape[0],
    ...
    # Open_Sigle_frame(): 只显示一帧，即：只显示 self.index 所指向的那一帧
    def Open_Single_Frame(self):
        # ret, image = self.cap.read()
        frame_dir = os.path.join(self.frame_path, self.frame_name_list[self.index])
        print(frame_dir)
        image = cv2.imread(frame_dir)

        # self.current_frame_lineEdit.setText(str(self.index+1))
        text = '%s / %s' % (str(self.index + 1), str(self.frame_len))
        self.current_all_frames_lineEdit.setText(text)
        # 滑动栏触发
        self.to_SN_Slider_signal.emit()

        # (1) 原图显示，只显示 self.index 所指向的那一帧
        image_height, image_width = image.shape[0], image.shape[1]
        long_w_h = image_width - image_height
        if long_w_h >= 0:
            #  w > h，加宽 h
    ...
    def previous_frame(self):
        if self.index < 1:
            ...
            self.index = self.index - 1
        self.pervious_frame_signal.emit()
    ...
    def SN_Slider_to_others(self):
        ...
        self.SN_Slider_to_others_signal.emit()
    ...
    def delete_files(delf, dir_path):
