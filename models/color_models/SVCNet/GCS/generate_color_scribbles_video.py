...
import models.color_models.SVCNet.GCS.pwcnet as pwcnet
...
def main_worker_color_scribbles(base_root, scribble_root, save_root, crop_size_h, crop_size_w):
    ...
    parser.add_argument('--pwcnet_path', type = str, \
        default = 'models/color_models/SVCNet/checkpoints/pwcNet-default.pytorch', \
    ...
    parser.add_argument('--saveroot', type = str, \
        default = "results/color_point40_color_width5_256p", \
            help = 'saveroot')
    ...
    imglist = sorted(get_files(base_root))
    print(imglist)
    ...
    for i in range(N):
        ...
        save_sub_folder_name = save_root
        check_path(save_sub_folder_name)

        save_resized_scribble_name = os.path.join(save_root, (imgpath.split('/')[-1].split('.')[0]+ '.png'))
        widen_s = cv2.cvtColor(widen_s, cv2.COLOR_BGR2RGB)
        cv2.imwrite(save_resized_scribble_name, widen_s)


        # save_original_size_scribble_name = os.path.join(save_root, (imgpath.split('/')[-1].split('.')[0] + '.png'))
        ...
        #
        # save_resized_scribble_name = os.path.join(save_root, (imgpath.split('/')[-1].split('.')[0] + '_' + str(opt.crop_size_h) + 'p_not_widen.png'))
        # cv2.imwrite(save_resized_scribble_name, s)
        #
        # save_original_size_scribble_name = os.path.join(save_root, (imgpath.split('/')[-1].split('.')[0] + '_not_widen.png'))
