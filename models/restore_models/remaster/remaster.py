...
import utils
import glob
from models.restore_models.remaster.utils import convertRGB2LABTensor, convertLAB2RGB
...
def main_worker_remaster(input=None,outputdir=None, reference_dir=None, disable_colorization=True, gpu=True, mindim=320):
   device = torch.device('cuda:0' if gpu else 'cpu')

   # Load remaster network
   modelR = __import__( 'models.restore_models.remaster.model.remasternet', fromlist=['NetworkR'] ).NetworkR()
   state_dict = torch.load( 'models/restore_models/remaster/checkpoints/remasternet.pth.tar' )
   ...
   if not disable_colorization:
      if reference_dir!='none':
         ...
         reference_files = []
         for ext in ext_list:
            reference_files += glob.glob( reference_dir+'/*.'+ext, recursive=True )
         aspect_mean = 0
         minedge_dim = 256
         refs = []
   ...
   frame_lst = sorted(glob.glob(os.path.join(input, '*')))
   # nframes = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) # cv2.CAP_PROP_FRAME_COUNT: 7
   nframes = len(frame_lst)
   # v_w = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
   v_w = cv2.imread(frame_lst[0]).shape[1]
   v_h = cv2.imread(frame_lst[0]).shape[0]
   ...
   with torch.no_grad():
      ...
      while True:
         ...
         for i in range(proc_g):
            index = frame_pos + i
            # _, frame = cap.read()
            frame = cv2.imread(frame_lst[index])
            frame = cv2.resize(frame, (t_w, t_h))
            nchannels = frame.shape[2]
            if nchannels == 1 or not disable_colorization:
               frame_l = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
               cv2.imwrite(outputdir_in+'/%05d.png'%index, frame_l)
               ...
               cv2.imwrite(outputdir_in+'/%05d.png'%index, frame)
            ...
            if nchannels==3 and disable_colorization:
         ...
         if disable_colorization:
            for i in range( proc_g ):
               ...
               if nchannels==3:
                  ...
                  out = Image.fromarray( np.uint8(convertLAB2RGB( out )*255 ) )
                  out.save( outputdir_out+'/%05d.png'%(index) )
               else:
                  save_image( output_l.detach()[0,:,i], outputdir_out+'/%05d.png'%(index), nrow=1 )
         # Perform colorization
            ...
            for i in range( proc_g ):
               ...
               output.save( outputdir_out+'/%05d.png'%index )

         it = it + 1
         pbar.update(proc_g)

      # Save result videos
      # outfile = input.split('/')[-1].split('.')[0]
      ...
      import shutil
      # shutil.rmtree(outputdir_in)
      # cap.release()
      pbar.close()

if __name__ == '__main__':
    main_worker_remaster(input='./001', outputdir='./testresult')